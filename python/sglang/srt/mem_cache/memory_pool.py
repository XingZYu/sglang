...
class MHATokenToKVPoolTurboQuant(MHATokenToKVPool):
    ...
    def get_tq_buffers(self, layer_id: int):
        """Return (k_packed, v_packed, k_scale, v_scale) for given layer_id."""
        idx = layer_id - self.start_layer
        return (
            self.k_buffer[idx],
            self.v_buffer[idx],
...
class HybridLinearKVPool(KVCache):
    ...
    def __init__(
        ...
        start_layer: Optional[int] = None,
        token_to_kv_pool_class=None,
        token_to_kv_pool_kwargs: Optional[dict] = None,
    ):
        self.size = size
        self.dtype = dtype
        self.device = device
        ...
        if not use_mla:
            ...
            if token_to_kv_pool_class is not None:
                TokenToKVPoolClass = token_to_kv_pool_class
            ...
            if token_to_kv_pool_kwargs:
                pool_kwargs.update(token_to_kv_pool_kwargs)
        ...
        kv_size = self.full_kv_pool.get_kv_size_bytes()
        if isinstance(kv_size, tuple):
            self.mem_usage = sum(kv_size) / GB
        else:
    ...
    def get_tq_buffers(self, layer_id: int):
        """Return (k_packed, v_packed, k_scale, v_scale) with hybrid layer_id mapping."""
        mapped_id = self._transfer_full_attention_id(layer_id)
        return self.full_kv_pool.get_tq_buffers(mapped_id)
